...
def test_categorical_td_target():
    ...
    next_logits = torch.tensor([[[0.0, 0.0, 0.0], [0.0, 100.0, 0.0]]]) # Next action 1 is certain at atom 1 (val 1.0)
    ...
    target_dist = categorical_td_target(
        next_logits, next_actions, rewards, terminated, gamma, support, v_min, v_max, atom_size
    )
