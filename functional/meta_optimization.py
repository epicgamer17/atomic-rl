...
def init_idbd_states(
    module: nn.Linear, init_lr: float = 0.01
) -> Dict[nn.Parameter, Dict[str, torch.Tensor]]:
    """
    Initializes the state dictionary for IDBD and its variants for a specific linear layer.

    Args:
        module (nn.Linear): The linear layer to track.
        init_lr (float): The initial learning rate (alpha). Beta will be initialized as ln(alpha).

    Returns:
        Dict mapping parameters to their IDBD states (betas, h, v).
    """
    states = {}
    init_beta = math.log(init_lr)

    for param in module.parameters():
        states[param] = {
            "betas": torch.full_like(param, init_beta, requires_grad=False),
            "h": torch.zeros_like(param, requires_grad=False),
            "v": torch.zeros_like(
                param, requires_grad=False
            ),  # Specifically for Autostep
        }
    return states


def apply_idbd_linear(
    module: nn.Linear,
    states: Dict[nn.Parameter, Dict[str, torch.Tensor]],
    inputs: torch.Tensor,
    error: torch.Tensor,
    algorithm: str = "autostep",  # TODO: avoid string use and use functions or functools.partial instead. Add this to our rules?
    meta_lr: float = 0.01,
    tau: float = 10000.0,
    r_hat: float = 1.0,
) -> None:
    """
    Orchestrates an IDBD update for a linear layer, replacing backward() and step().

    Args:
        module (nn.Linear): The linear layer being updated.
        states (Dict): The IDBD states generated by `init_idbd_states`.
        inputs (torch.Tensor): The raw features that were fed into the module. Shape: [in_features].
        error (torch.Tensor): The scalar prediction error (Target - Prediction). Shape: [1] or [out_features].
        algorithm (str): The variant to run. Options: "autostep" (default), "idbd", "k1", "k2".
        meta_lr (float): Meta learning rate for adapting betas.
        tau (float): Time constant for the normalizer decay (Autostep only).
        r_hat (float): Estimate of observation noise variance (K1 and K2 only).
    """
    # Fail Fast: IDBD is fundamentally an online (step-by-step) algorithm.
    assert (
        inputs.dim() == 1
    ), f"IDBD expects online unbatched inputs [Features], got {inputs.shape}"
    assert error.dim() == 1, f"Error must be 1D [Out_Features], got {error.shape}"
    # NOTE: this is for paper faithfulness
    # TODO: make bias be properly handled for all implementations (including AutoStep)
    assert module.bias is None, (
        "IDBD/Autostep expects bias=False. "
        "If an intercept is needed, augment the input with a constant 1-valued feature."
    )
    # Broadcast inputs to match weight matrix [out_features, in_features]
    w_inputs = inputs.unsqueeze(0).expand_as(module.weight)
    # NOTE: This implementation extends IDBD/Autostep from scalar linear prediction to multi-output linear layers by applying independent adaptive step-sizes per output-feature weight
    # TODO: make tests to verify this works otherwise remove this extension
    w_error = error.unsqueeze(1).expand_as(module.weight)

    with torch.no_grad():

        # Internal helper to route to the correct pure math function
        def _update_parameter(
            param: nn.Parameter, p_inputs: torch.Tensor, p_error: torch.Tensor
        ):
            p_state = states[param]

            if algorithm == "autostep":
                new_w, new_b, new_h, new_v, _ = compute_autostep_idbd_update(
                    weights=param,
                    betas=p_state["betas"],
                    h=p_state["h"],
                    v=p_state["v"],
                ...
                p_state["v"].copy_(new_v)
                p_state["h"].copy_(new_h)

            elif algorithm == "idbd":
                new_w, new_b, new_h, _ = compute_idbd_update(
                    weights=param,
                    betas=p_state["betas"],
                    h=p_state["h"],
                    inputs=p_inputs,
                ...
                new_w, new_b, new_h, _ = compute_k1_update(
                    weights=param,
                    betas=p_state["betas"],
                    h=p_state["h"],
                    inputs=p_inputs,
                ...
                new_w, new_b, _ = compute_k2_update(
                    weights=param,
                    betas=p_state["betas"],
                    inputs=p_inputs,
                    error=p_error,
                ...
                # K2 drops the `h` trace entirely, no h update needed

            else:
                raise ValueError(
                    f"Unknown IDBD algorithm: '{algorithm}'. Valid options: 'autostep', 'idbd', 'k1', 'k2'"
                )

            # Update the weight and beta (common to all algorithms)
            param.copy_(new_w)
            p_state["betas"].copy_(new_b)

        # 1. Update Weights
        _update_parameter(module.weight, w_inputs, w_error)


def compute_idbd_update(
    ...
    """
    Computes a single step of the Incremental Delta-Bar-Delta (IDBD) algorithm.
    ...
    Args:
        weights (torch.Tensor): Current weights. Shape: [Batch, Features] or [Features].
        betas (torch.Tensor): Log-learning rates (ln(alpha)). Proportional to the correlation between current and recent weight changes. Shape must match weights.
        h (torch.Tensor): Decaying trace of the cumulative sum of recent changes to weights. Shape must match weights.
        inputs (torch.Tensor): The input features (x). Shape must match weights.
    ...
    Returns:
        Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
            - new_weights: The updated weights.
    ...
    assert (
        weights.shape == betas.shape == h.shape == inputs.shape
    ), f"Shape mismatch: weights({weights.shape}), betas({betas.shape}), h({h.shape}), inputs({inputs.shape})"
    ...
    # 1. Update betas (log learning rates)
    # Equation: beta(t+1) = beta(t) + theta * error * input * h(t)
    delta_beta = meta_lr * error * inputs * h
    delta_beta = torch.clamp(delta_beta, min=-max_beta_change, max=max_beta_change)
    new_betas = betas + delta_beta
    new_betas = torch.clamp(new_betas, min=min_beta, max=None)
    ...
    # 3. Update weights
    # Equation: w(t+1) = w(t) + alpha(t+1) * error * input
    new_weights = weights + alphas * error * inputs

    # 4. Update the trace (h)
    # Equation: decay = max(1 - alpha(t+1) * input^2, 0)
    # We use max(..., 0) to prevent instability if the step size gets too large.
    ...
    return new_weights, new_betas, new_h, alphas


def compute_k1_update(
    weights: torch.Tensor,
    betas: torch.Tensor,
    h: torch.Tensor,
    inputs: torch.Tensor,
    ...
    Args:
        weights (torch.Tensor): Current weights. Shape: [..., Features].
        betas (torch.Tensor): Log-learning rates. Shape must match weights.
        h (torch.Tensor): Memory trace of past updates. Shape must match weights.
        inputs (torch.Tensor): Input features. Shape must match weights.
    ...
    # 5. Update weights
    new_weights = weights + k_gain * error
    ...
    return new_weights, new_betas, new_h, k_gain


def compute_k2_update(
    weights: torch.Tensor,
    betas: torch.Tensor,
    inputs: torch.Tensor,
    error: torch.Tensor,
    ...
    """
    assert (
        weights.shape == betas.shape == inputs.shape
    ), f"Shape mismatch: weights({weights.shape}), betas({betas.shape}), inputs({inputs.shape})"
    if inputs.dim() > 1:
        assert (
            error.dim() == inputs.dim() and error.shape[-1] == 1
        ), f"Batched error must have shape [..., 1], got {error.shape}"
    ...
    return new_weights, new_betas, k_gain
...
def compute_autostep_idbd_update(
    weights: torch.Tensor,
    betas: torch.Tensor,
    h: torch.Tensor,
    v: torch.Tensor,
    ...
    """
    Computes Autostep applied to the base IDBD algorithm.
    Initialization:
    ...
    # 5. Execute standard weight and trace updates using the safe alphas
    new_weights = weights + new_alphas * error * inputs
    ...
    return new_weights, new_betas, new_h, new_v, new_alphas
